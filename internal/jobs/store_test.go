...
func TestStore_TryEnqueue_ConflictSameService(t *testing.T) {
	s := NewStore()
	j1, ex, err := s.TryEnqueue("web", ActionUpdate)
	...
	j2, ex2, err2 := s.TryEnqueue("web", ActionRestart)
	...
	_, _, err3 := s.TryEnqueue("api", ActionRestart)
	if err3 != nil {
...
func TestStore_Finish_ReleasesService(t *testing.T) {
	s := NewStore()
	j, _, _ := s.TryEnqueue("web", ActionUpdate)
	s.MarkRunning(j.ID)
	s.FinishSucceeded(j.ID, "ok", "")

	_, ex, err := s.TryEnqueue("web", ActionRestart)
	if err == ErrConflict {
